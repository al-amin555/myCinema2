...
export default function CartDetails({ onCloseCart }) {
  ...
  const handleRemove = (e, cartItem) => {
    // const filtered = state.cartData.filter((item) => item.id !== cartId);
    e.preventDefault();
    dispatch({
      type: "removed",
      payload: cartItem,
    ...
    toast.warn(`The ${cartItem.title} has been deleted!`, {
      position: "top-right",
      autoClose: 2000,
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[420px] sm:max-w-[600px] lg:max-w-[790px] p-4 max-h-[90vh] overflow-auto">
          <div className="bg-white shadow-md dark:bg-[#12141D] rounded-2xl overflow-hidden p-5 md:p-9">
            ...
            {state.cartData.length === 0 && (
              <p className="text-center text-2xl"> There is nothing added</p>
            )}
